...
{
  return fit(smoothMod(seed * 8., 1.0, 1.5), 0.35, 0.6, 0.0, 1.0);
}

void	main()
{
	float pattern = wave(noise(normal + vec3(u_time * (1. - u_mouse.x * u_mouse.y * .5) * .2 )));
	...
	vec3 newPosition = position + position * (pattern - .5) * .1;

	gl_Position = projectionMatrix * modelViewMatrix * vec4( newPosition, 1.0);
}
`;
export { sphereVertexShader };
